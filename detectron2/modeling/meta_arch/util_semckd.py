    ...
    def forward(self, feat_s, feat_t):
        ...
        for i in range(len(sim_s)):
            ...
            for j in range(len(sim_t)):            
                s_H, s_W, t_H, t_W = feat_s[i].shape[2], feat_s[i].shape[3], feat_t[j].shape[2], feat_t[j].shape[3]
                if s_H > t_H:
                    input = F.adaptive_avg_pool2d(feat_s[i], (t_H, t_W))
                    proj_value_stu[i].append(getattr(self, 'regressor'+str(i)+str(j))(input))
                    value_tea[i].append(feat_t[j])
                elif s_H < t_H or s_H == t_H:
                    target = F.adaptive_avg_pool2d(feat_t[j], (s_H, s_W))
