...
class RPN_SAKD(nn.Module):
    ...
    @torch.no_grad()
    def label_and_sample_anchors(self, anchors: List[Boxes], gt_instances: List[Instances], instance_mask):
        """
        Args:
    ...
    def icc(self, f):  ##inter channel correlation
        # mean(1) function reduce feature map BxCxHxW into BxHxW by averaging the channel response
        f=f.view(f.size(0), f.size(1), -1)
        return F.normalize(f @ f.permute(0, 2, 1))

    def ca(self, f):  ##channel attention
        # mean(1) function reduce feature map BxCxHxW into BxHxW by averaging the channel response
        f_ca=F.normalize(f.pow(2).mean(dim=(2,3), keepdim=True))
        return f*f_ca

    def sa(self, f):  ##channel attention
        # mean(1) function reduce feature map BxCxHxW into BxHxW by averaging the channel response
        f_sa = F.normalize((f.pow(2).mean(dim=(1), keepdim=True)), dim=(2, 3))
        return f*f_sa
    ...
    def losses(
        self
            ...
            #original
            loss_p2 = criterion(features['p2'], teacher_features['p2'])
            loss_p3 = criterion(features['p3'], teacher_features['p3'])
            loss_p4 = criterion(features['p4'], teacher_features['p4'])
            loss_p5 = criterion(features['p5'], teacher_features['p5'])


            loss_p2 = loss_p2.mean(dim=1)
            loss_p3 = loss_p3.mean(dim=1)
            loss_p4 = loss_p4.mean(dim=1)
            loss_p5 = loss_p5.mean(dim=1)

            gt_boxes = [x.gt_boxes for x in gt_instances]
            instance_num = [gt_box.tensor.size(0) for gt_box in gt_boxes]
            max_instance = max(instance_num)



            loss_p2 = (loss_p2.unsqueeze(dim=1)).repeat_interleave(max_instance, dim=1)
            loss_p3 = (loss_p3.unsqueeze(dim=1)).repeat_interleave(max_instance, dim=1)
            loss_p4 = (loss_p4.unsqueeze(dim=1)).repeat_interleave(max_instance, dim=1)
            loss_p5 = (loss_p5.unsqueeze(dim=1)).repeat_interleave(max_instance, dim=1)
    ...
    def forward(
        ...
        gt_instances: Optional[Instances],
        instance_mask,
        class_weight,
    ):
        """
        ...
        """
        rpnfeatures = [features[f] for f in self.in_features]

        anchors = self.anchor_generator(rpnfeatures)

        pred_objectness_logits, pred_anchor_deltas = self.rpn_head(rpnfeatures)
