    ...
    for line in image_set:
        ...
        if subset == 'train':
            ...
            for ann_ind, txt in enumerate(anns):
                ...
                instance_mask_p2 = np.zeros((int(image.shape[0] / 4), int(image.shape[1]/4)), dtype=bool)
                instance_mask_p3 = np.zeros((int(image.shape[0] / 8), int(image.shape[1] / 8)), dtype=bool)
                instance_mask_p4 = np.zeros((int(image.shape[0] / 16), int(image.shape[1] / 16)), dtype=bool)
                instance_mask_p5 = np.zeros((int(image.shape[0] / 32), int(image.shape[1] / 32)), dtype=bool)


                for ih in range(int(image.shape[0])):
                    for iw in range(int(image.shape[1])):
                        if (bbox[1] <= ih) and (bbox[3] >= ih) and (bbox[0] <= iw) and (bbox[2] >= iw):
                            instance_mask_p2[int(ih / 4)][int(iw / 4)] = 1
                            instance_mask_p3[int(ih / 8)][int(iw / 8)] = 1
                            instance_mask_p4[int(ih / 16)][int(iw / 16)] = 1
                            instance_mask_p5[int(ih / 32)][int(iw / 32)] = 1
                ...
                ann = {'image_id': image_id,
                    ...
                    'instance_mask_p2' : instance_mask_p2.tolist(),
                    'instance_mask_p3': instance_mask_p3.tolist(),
                    'instance_mask_p4': instance_mask_p4.tolist(),
                    'instance_mask_p5': instance_mask_p5.tolist()}
