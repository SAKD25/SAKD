...
class ALLROIHeads_teacher(ROIHeads):
    """
    ...
    def forward(
        ...
        if 1:
            if 0: #teacher proposal
                proposals, batch_sample_inxs = self.label_and_sample_proposals(proposals, targets)
            del targets

            logits, teacher_roi_features = self._forward_box(features, proposals)
            # Usually the original proposals used by the box head are used by the mask, keypoint
            # heads. But when `self.train_on_pred_boxes is True`, proposals will contain boxes
            # predicted by the box head.
            # losses.update(self._forward_att(features, proposals))
            #losses.update(self._forward_dis(features, pos_proposals))
            #box_features = self._forward_dis(features, pos_proposals)
            #distance_logits = (self._forward_dis(features, proposals))
            return logits, teacher_roi_features #, proposals, batch_sample_inxs
        else:
    ...
    def _forward_box(
        ...
        box_features_head = self.box_head(box_features)
        predictions = self.box_predictor(box_features_head)
        del box_features_head
