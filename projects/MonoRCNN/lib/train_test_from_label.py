...
def load_json(json_file, image_root, cfg):
    ...
    logger.info("car samples: {}".format(car_samples))
    logger.info("ped samples: {}".format(ped_samples))
    logger.info("cyc samples: {}".format(cyc_samples))
    logger.info("dontcare samples: {}".format(dontcare_samples))
...
def model_pred_function(image, model, cfg):
    ...
    if cfg.MODEL.META_ARCHITECTURE == "GeneralizedRCNN_SAKD":
        predictions = model([inputs],None,None,None)[0]
    else:
...
def inference_on_dataset(cfg, model, data_loader, iteration):
    ...
    with inference_context(model), torch.no_grad():
        for idx, dataset_dict in enumerate(data_loader):
            ...
            file = open(os.path.join(result_dir, file_name.split('/')[-1][:-4] + '.txt'), 'r')

            SimKD_file = open(os.path.join(result_dir.split('/')[0],result_dir.split('/')[1],result_dir.split('/')[2],result_dir.split('/')[3],'label2', file_name.split('/')[-1][:-4] + '.txt'), 'r')
            gt_file = open(os.path.join(result_dir.split('/')[0],'Vistest_SimKD',result_dir.split('/')[2],result_dir.split('/')[3],'label2', file_name.split('/')[-1][:-4] + '.txt'), 'r')
            text_to_write = ''
            ...
            if(1):
                B3KD_lines = file.readlines()

                for anno_line in anno_lines:  #

                    cla = anno_line.split(' ')[0]
                    alpha = float(anno_line.split(' ')[3])
                    x1 = float(anno_line.split(' ')[4])
                    y1 = float(anno_line.split(' ')[5])
                    x2 = float(anno_line.split(' ')[6])
                    y2 = float(anno_line.split(' ')[7])
                    h3d = float(anno_line.split(' ')[8])
                    w3d = float(anno_line.split(' ')[9])
                    l3d = float(anno_line.split(' ')[10])
                    x3d = float(anno_line.split(' ')[11])
                    y3d = float(anno_line.split(' ')[12])
                    z3d = float(anno_line.split(' ')[13])
                    ry3d = float(anno_line.split(' ')[14])
    ...
    #return '../../output/evaluation/04999/data', iteration
